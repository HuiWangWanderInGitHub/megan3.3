...
!-------------------------------------------------------------
   ...
   subroutine area_interp( lon_edge, lat_edge, nlon, nlat, missing_value, &
                           wrk_data, ncid, varname, grid_ndx, new_grid , flip_flag)
!-------------------------------------------------------------
...
!-------------------------------------------------------------
   ...
   integer, allocatable :: megan_data(:,:)
   integer, allocatable :: megan_data_orig(:,:)
...
!-------------------------------------------------------------
   ...
   if( allocated( megan_data_orig ) ) then
     deallocate( megan_data_orig )
   endif

   allocate( megan_data(nlon,nlat), megan_data_orig(nlon,nlat), stat=astat )

   if( astat /= 0 ) then
     write(*,*) 'area_interp: Failed to allocate megan_data; error = ',astat
   ...
   call handle_ncerr( nf_get_var_int( ncid, varid, megan_data_orig ), message )
   where( megan_data_orig(:,:) == missing_value )
     megan_data_orig(:,:) = 0_2
   endwhere
!flip data based on flipped latitude
   if( flip_flag ) then
    megan_data = megan_data_orig(:,nlat:1:-1)
   else
    megan_data = megan_data_orig
   end if
...
inside_mcell : &
   ...
   write(*,'(''area_interp: min model lat ndx = '',i10)') minval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''area_interp: max model lat ndx = '',i10)') maxval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''area_interp: mcells outside data domain = '',i12)') count( .not. model_area_type(:,:)%has_data )
...
!  stop 'diagnostics'
   ...
   subroutine discrete_frac( lon_edge, lat_edge, nlon, nlat, missing_value, &
                           wrk_data, ncid, varname, grid_ndx, new_grid,flip_flag )
!-------------------------------------------------------------
!  ... area conserving interpolation from data to wrf grid
!-------------------------------------------------------------
...
!-------------------------------------------------------------
   ...
   integer :: ietype
   integer :: mxetype = 100
   integer :: wrkid_ntypes
   !integer(2), allocatable :: megan_data(:,:)
   integer, allocatable :: megan_data(:,:)
   integer, allocatable :: megan_data_org(:,:)
   integer, allocatable :: dcell_partial_lon_ndx(:)
   integer, allocatable :: dcell_partial_lat_ndx(:)
   integer, allocatable :: wrkid(:)
   integer, allocatable :: wrkid_cnt(:)
   real    :: dcell_area
   real    :: wrk_sum
   real, allocatable    :: partial_wght(:)
   logical :: dcell_outside_mcell
...
!-------------------------------------------------------------
   ...
   if( allocated( megan_data_org ) ) then
     deallocate( megan_data_org )
   endif
   
   allocate( megan_data(nlon,nlat), megan_data_org(nlon,nlat), stat=astat )
   


   if( astat /= 0 ) then
     write(*,*) 'discrete_frac: Failed to allocate megan_data; error = ',astat
     stop 'allocation error'
   endif
   message = 'area_map: Failed to get variable id'
   call handle_ncerr( nf_inq_varid( ncid, trim(varname), varid ), message )
   message = 'Failed to read variable'
   !call handle_ncerr( nf_get_var_int2( ncid, varid, megan_data ), message )
   call handle_ncerr( nf_get_var_int( ncid, varid, megan_data_org ), message )
!flip data based on flipped latitude
    print*,"Flip_grid:",flip_flag
   if( flip_flag ) then
    megan_data = megan_data_org(:,nlat:1:-1)
   else
    megan_data = megan_data_org(:,:)
   end if
   print*,"missing_value:",missing_value
   where( megan_data(:,:) == missing_value .or. megan_data(:,:) == -9999)
     megan_data(:,:) = -999
   endwhere
...
!-------------------------------------------------------------
         ...
         allocate( dcell_partial_lon_ndx(dcell_cnt), dcell_partial_lat_ndx(dcell_cnt), &
                   partial_wght(dcell_cnt),stat=astat )
         if( astat /= 0 ) then
           write(*,*) 'discrete_frac; failed to allocate dcell_partial_lon_ndx ... partial_wght: error = ',astat
           stop
         endif
       else is_new_grid
         lon_s = model_area_type(mlon_ndx,mlat_ndx)%lon_s
       ...
       allocate(wrkid(mxetype),wrkid_cnt(mxetype),stat=astat)
       if( astat /= 0 ) then
         write(*,*) 'discrete_frac; failed to allocate wrkid = ',astat
         stop
       endif
!!!    initialize 1-D variables before reading in data cells for this model grid
!!!    cell
       wrkid(:) = 0
       wrkid_cnt(:) = 0
       wrkid_ntypes = 0
...
inside_mcell : &
           if( .not. dcell_outside_mcell ) then
             ...
             dcell_cnt1 = dcell_cnt1 + 1
!!!
!!!          Store ecotype IDs and count # of grid cells for each one separately
!!!
             typefound = .false.
             do ietype = 1,mxetype
               if (megan_data(i,j) == wrkid(ietype)) then
                  wrkid_cnt(ietype) = wrkid_cnt(ietype) + 1
                  typefound = .true.
                  exit
               endif
             end do
!!!          If new ecotype, fill next array index
             if (.not. typefound) then
               wrkid_ntypes = wrkid_ntypes + 1
               wrkid(wrkid_ntypes) = megan_data(i,j)
               wrkid_cnt(wrkid_ntypes) = 1
             endif
           endif inside_mcell
         end do data_lon_loop
       end do data_lat_loop
!!!
!!!    Store ecotype fractions and ecotypeIDs for each model grid cell
!!!
       wrk_data(mlon_ndx,mlat_ndx,1:mxetype,1) = wrkid(:)
       wrk_data(mlon_ndx,mlat_ndx,1:mxetype,2) = real(wrkid_cnt(:))/real(dcell_cnt1)
       if( new_grid .and. dcell_ndx > 0 ) then
         model_area_type(mlon_ndx,mlat_ndx)%partial_dcell_cnt = dcell_ndx
         allocate( model_area_type(mlon_ndx,mlat_ndx)%dcell_lon_ndx(dcell_ndx), &
                   model_area_type(mlon_ndx,mlat_ndx)%dcell_lat_ndx(dcell_ndx), &
                   model_area_type(mlon_ndx,mlat_ndx)%wght(dcell_ndx),stat=astat )
         if( astat /= 0 ) then
           write(*,*) 'discrete_frac; failed to allocate model type dcell_partial_lon_ndx ... partial_wght: error = ',astat
           stop
         endif
         model_area_type(mlon_ndx,mlat_ndx)%dcell_lon_ndx(:dcell_ndx) = dcell_partial_lon_ndx(:dcell_ndx)
         model_area_type(mlon_ndx,mlat_ndx)%dcell_lat_ndx(:dcell_ndx) = dcell_partial_lat_ndx(:dcell_ndx)
   ...
   write(*,*) ' '
   write(*,*) 'discrete_frac: diagnostics'
   write(*,'(''discrete_frac: min model lat ndx = '',i10)') minval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''discrete_frac: max model lat ndx = '',i10)') maxval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''discrete_frac: mcells outside data domain = '',i12)') count( .not. model_area_type(:,:)%has_data )
   write(*,'(''discrete_frac: wght size = '',1pg22.15)') mcell_area
   write(*,'(''discrete_frac: total dcell cnt            = '',i9)') sum( model_area_type(:,:)%total_dcell_cnt )
   write(*,'(''discrete_frac: active dcell cnt           = '',i9)') sum( model_area_type(:,:)%active_dcell_cnt )
   write(*,'(''discrete_frac: interior dcell cnt         = '',i9)') sum( model_area_type(:,:)%interior_dcell_cnt )
   write(*,'(''discrete_frac: partial dcell cnt          = '',i9)') sum( model_area_type(:,:)%partial_dcell_cnt )

!  stop 'diagnostics'

   end subroutine discrete_frac
...
!---------------------------------------------------------------
   ...
   if( nv > 3 .and. vtx_cnt > 1 ) then
     write(*,'('' area_map: there are '',i1,'' model vertices in the data cell'')') vtx_cnt
       ...
       do m = 1,4
         write(*,'('' ('',g16.8,'','',g16.8,'')'')') model_x(m),model_y(m)
       end do
       write(*,*) ' '
       write(*,*) 'area_map: model cell vertices(lon,lat)'
       do m = 1,4
         write(*,'('' ('',g16.8,'','',g16.8,'')'')') mvtx_lon(m),mvtx_lat(m)
       end do
       write(*,*) ' '
       write(*,*) 'area_map: data cell vertices(x,y)'
       do m = 1,4
         write(*,'('' ('',g16.8,'','',g16.8,'')'')') x(m),y(m)
       end do
       write(*,*) ' '
       write(*,*) 'area_map: data cell vertices(lon,lat)'
       do m = 1,4
         write(*,'('' ('',g16.8,'','',g16.8,'')'')') data_lon(m),data_lat(m)
       end do
       write(*,*) ' '
       ...
       write(*,*) ' '
       do m = 1,nv
         write(*,'('' ('',g16.8,'','',g16.8,'')'')') vtx_x(m),vtx_y(m)
       end do
       write(*,*) '##########################################################'
     stop
   endif
...
!---------------------------------------------------------------------
   ...
   subroutine handle_ncerr_r1( ret, var_flag )
!---------------------------------------------------------------------
!	... netcdf error handling routine
!---------------------------------------------------------------------
   integer, intent(in) :: ret
   logical, intent(inout) :: var_flag
   var_flag = .False. 

   if( ret /= nf_noerr ) then
      var_flag = .True. 
   endif
   end subroutine handle_ncerr_r1



   subroutine area_interp2( lon_edge, lat_edge, nlon, nlat, missing_value, &
                           wrk_data, ncid, varname, grid_ndx, new_grid, flip_flag )
!-------------------------------------------------------------
!  ... area conserving interpolation from data to wrf grid
!-------------------------------------------------------------
...
!-------------------------------------------------------------
   ...
   integer :: dcell_ndx
   !integer(2), allocatable :: megan_data(:,:)
   real, allocatable :: megan_data(:,:)
   real, allocatable :: megan_data_orig(:,:)
   integer, allocatable :: dcell_partial_lon_ndx(:)
   integer, allocatable :: dcell_partial_lat_ndx(:)
   real    :: dcell_area
   real    :: wrk_sum
...
!-------------------------------------------------------------
   ...
   allocate( megan_data(nlon,nlat), megan_data_orig(nlon,nlat), stat=astat )
   if( astat /= 0 ) then
     write(*,*) 'area_interp2: Failed to allocate megan_data; error = ',astat
     stop 'allocation error'
   endif
   message = 'area_map: Failed to get variable id'
   call handle_ncerr( nf_inq_varid( ncid, trim(varname), varid ), message )
   message = 'Failed to read variable'
   call handle_ncerr( nf_get_var_real( ncid, varid, megan_data_orig ), message )
   where( megan_data_orig(:,:) == missing_value )
     megan_data_orig(:,:) = 0_2
   endwhere
...
!-------------------------------------------------------------
         ...
         allocate( dcell_partial_lon_ndx(dcell_cnt), dcell_partial_lat_ndx(dcell_cnt), &
                   partial_wght(dcell_cnt),stat=astat )
         if( astat /= 0 ) then
           write(*,*) 'area_interp2; failed to allocate dcell_partial_lon_ndx ... partial_wght: error = ',astat
           stop
         endif
       else is_new_grid
         lon_s = model_area_type(mlon_ndx,mlat_ndx)%lon_s
...
inside_mcell : &
       ...
       if( new_grid .and. dcell_ndx > 0 ) then
         ...
         allocate( model_area_type(mlon_ndx,mlat_ndx)%dcell_lon_ndx(dcell_ndx), &
                   ...
                   model_area_type(mlon_ndx,mlat_ndx)%wght(dcell_ndx),stat=astat )
         if( astat /= 0 ) then
           write(*,*) 'area_interp2; failed to allocate model type dcell_partial_lon_ndx ... partial_wght: error = ',astat
           stop
         endif
         model_area_type(mlon_ndx,mlat_ndx)%dcell_lon_ndx(:dcell_ndx) = dcell_partial_lon_ndx(:dcell_ndx)
         model_area_type(mlon_ndx,mlat_ndx)%dcell_lat_ndx(:dcell_ndx) = dcell_partial_lat_ndx(:dcell_ndx)
   ...
   write(*,*) ' '
   write(*,*) 'area_interp2: diagnostics'
   write(*,'(''area_interp2: min model lat ndx = '',i10)') minval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''area_interp2: max model lat ndx = '',i10)') maxval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''area_interp2: mcells outside data domain = '',i12)') count( .not. model_area_type(:,:)%has_data )
   write(*,'(''area_interp2: wght size = '',1pg22.15)') mcell_area
   write(*,'(''area_interp2: total dcell cnt            = '',i9)') sum( model_area_type(:,:)%total_dcell_cnt )
   write(*,'(''area_interp2: active dcell cnt           = '',i9)') sum( model_area_type(:,:)%active_dcell_cnt )
   write(*,'(''area_interp2: interior dcell cnt         = '',i9)') sum( model_area_type(:,:)%interior_dcell_cnt )
   write(*,'(''area_interp2: partial dcell cnt          = '',i9)') sum( model_area_type(:,:)%partial_dcell_cnt )

!  stop 'diagnostics'

   end subroutine area_interp2

   end module area_mapper_grw
