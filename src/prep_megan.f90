...
contains
   ...
   subroutine prep(wrffile,nlai,lai_scale_factor,                    &
                   ...
                   soil_on, nitro_file, fert_file, climate_file, landtype_file,idxs)
     implicit none
     ...
     !Static data:
     call prep_static_data(idxs,ide,jde,canopy_file,growtype_file,ecotypes_file,GtEcoEF_file,climate_file,landtype_file, soil_on)
          ...
          ! `arid`     (soil interface): arid soils mask
          ! `landtype` (soil interface): land type classification
   
     !Time/date dependent data:
     call prep_dynamic_data(idxs,ide,jde,laiv_file,nlai,lai_scale_factor,nitro_file,fert_file,soil_on) 
     !subroutine prep_dynamic_data(idxs,ide,jde,laiv_file,nlai,lai_scale_factor,nitro_file, fert_file,soil_on)
          ! `LAI`     monthly.         : Leaf Area Index
          ! `N_DEP:`  monthly. (soil interface) : Nitrogen deposition flux
          ! `N_FERT:` daily.   (soil interface) : Nitrogen fertilization flux
 ...
 !  STATIC  DATA:
 !--------------------------------
    subroutine prep_static_data(idxs,ide,jde,canopy_file,ctf_file, ecotype_file, GtEcoEF_file, climate_file,landtype_file, soil_on)
      ...
      call create_static_file(outfile,idxs,soil_on)
 ...
 !---------------------------------
    subroutine prep_dynamic_data(idxs,ide,jde,laiv_file,nlai,lai_scale_factor,nitro_file, fert_file,soil_on)
      ...
      print*, "========================================="
      call create_dynamic_file(out_dyn_file,idxs,nlai,soil_on)
...
!=======================================================================
   subroutine create_static_file(outfile,idxs,soil_on)
     use netcdf
     use area_mapper_grw, only: lon, lat
     implicit none
     ...
     call check(nf90_create(outfile, IOR(NF90_CLOBBER, NF90_NETCDF4), ncid))
       ...
       "Canopy Type Fraction:1.Needleleaf Trees;2. Tropical Trees; 3.Broadleaf Tree;4. Shrub;5. Herb;6.  Crop;7. Total Tree Fraction" ))
        ...
        call check(nf90_put_att(ncid, var_id,"var_desc" , "Ligth Dependent Emissions Factors: LDF01,...LDF04" ))
        if (soil_on) then
           print*,"Building soil ARID and LANDTYPE fields ..."
           ! LANDTYPE and ARID (soil interface)
...
!==============================================================
!==============================================================
   subroutine create_dynamic_file(outfile,idxs,nlai,soil_on)
     use netcdf
     use area_mapper_grw, only: lon, lat
     implicit none
     character(len=*),intent(in) :: outfile  
