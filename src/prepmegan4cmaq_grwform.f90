...
!--------------------------------------------------------------------
   ...
   use area_mapper_grw, only : xlong => lon, xlat => lat
   use bio_types
   ...
   real, allocatable :: megan_lats_orig(:)
!----------------------------------------------------
!----allocate pft arrays-----------------------------
...
!growthform
   CHARACTER (LEN=80)  :: treefilename 
   CHARACTER (LEN=80)  :: treevarname 
   CHARACTER (LEN=80)  :: treefillvalue
   CHARACTER (LEN=80)  :: shrubfilename 
   CHARACTER (LEN=80)  :: shrubvarname 
   CHARACTER (LEN=80)  :: shrubfillvalue
   CHARACTER (LEN=80)  :: grassfilename 
   CHARACTER (LEN=80)  :: grassvarname 
   CHARACTER (LEN=80)  :: grassfillvalue
   CHARACTER (LEN=80)  :: cropfilename 
   CHARACTER (LEN=80)  :: cropvarname 
   CHARACTER (LEN=80)  :: cropfillvalue
   ...
   namelist /growthform/ treefilename,treevarname,treefillvalue, & !hw
                         shrubfilename,shrubvarname, shrubfillvalue, &
                         grassfilename,grassvarname, grassfillvalue, &
                         cropfilename,cropvarname, cropfillvalue
...
!-----------------------------------------------------------------
   ...
   read(*,nml=growthform,iostat=ierr)
   if( ierr /= 0 ) then
     write(*,*) 'convert_emissions: failed to read namelist; error = ',ierr
     stop 'bio_emiss abort'
   endif
...
!---------------------------------------------------------------------
   ...
   use area_mapper_grw, only : proj_init
...
!---------------------------------------------------------------------
   ...
   use area_mapper_grw, only : area_interp
...
!-------------------------------------dimid---------------------------
    ...
    call handle_ncerr_r1( nf_inq_dimid( ncid, 'lon', dimid ), var_flag )
    if( var_flag )then
...
!---------------------------------------------------------------------
    ...
    call handle_ncerr_r1( nf_inq_dimid( ncid, 'lat', dimid ), var_flag )
    if( var_flag )then
...
!---------------------------------------------------------------------
    ...
    call handle_ncerr_r1( nf_inq_varid( ncid, 'lon', varid ), var_flag )
    if( var_flag )then
    ...
    if( allocated( megan_lats_orig ) ) then
      deallocate( megan_lats_orig )
    endif

    allocate( megan_lats(nlat_megan),stat=ierr )
    allocate( megan_lats_orig(nlat_megan),stat=ierr )
...
!---------------------------------------------------------------------
    ...
    call handle_ncerr_r1( nf_inq_varid( ncid, 'lat', varid ), var_flag )
    if( var_flag )then
         call handle_ncerr( nf_inq_varid( ncid, 'y', varid ), message )
         message = 'megan2_bioemiss: Failed to read lon variable'
    end if
!flip the latitude to make it increasing
    call handle_ncerr( nf_get_var_real( ncid, varid, megan_lats_orig ), message )
    flip_flag = .False.
    grid_dy = megan_lats_orig(2)-megan_lats_orig(1)
    print*,"dy(lat) =:",grid_dy
    if( grid_dy .le. 0)then
        print*, "Latitude will be flipped"
    ...
    !megan_lats = megan_lats_orig(16800:1:-1)
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
      ...
      if( varname(:) == trim(treevarname)) then
        growthform_01(:,:) = scale_factor * wrk_data(:,:)
     elseif( varname(:) == trim(shrubvarname) ) then
        growthform_02(:,:) = scale_factor * wrk_data(:,:)
     elseif( varname(:) == trim(cropvarname) ) then
        growthform_03(:,:) = scale_factor * wrk_data(:,:)
     elseif( varname(:) == trim(grassvarname) ) then
...
!---------------------------------------------------------------------
     ...
     do ilat=1,jde-jds+1
        do ilon=1,ide-ids+1
        if(TreeFrac(ilon,ilat).gt.1.or.TreeFrac(ilon,ilat).lt.0.0)TreeFrac(ilon,ilat)=0.0
        if(ShrubFrac(ilon,ilat).gt.1.or.ShrubFrac(ilon,ilat).lt.0.0)ShrubFrac(ilon,ilat)=0.0
        if(CropFrac(ilon,ilat).gt.1.or.CropFrac(ilon,ilat).lt.0.0)CropFrac(ilon,ilat)=0.0
        if(GrassFrac(ilon,ilat).gt.1.or.GrassFrac(ilon,ilat).lt.0.0)GrassFrac(ilon,ilat)=0.0
        end do
      end do
